  ...
  try {
    ...
    if (existing) {
      return NextResponse.json({
        id: existing.id,
        code: existing.code,
        url: `${process.env.NEXT_PUBLIC_APP_URL || 'https://scout.cornell.edu'}/invite/${existing.code}`,
      })
    }
    ...
    return NextResponse.json({
      id: data.id,
      code: data.code,
      url: `${process.env.NEXT_PUBLIC_APP_URL || 'https://scout.cornell.edu'}/invite/${data.code}`,
    })
  } catch (err) {
