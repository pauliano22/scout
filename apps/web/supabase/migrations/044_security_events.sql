...
CREATE POLICY "Service role only — select events"
  ON public.security_events FOR SELECT
  USING (auth.role() = 'service_role');
CREATE POLICY "Service role only — insert events"
  ON public.security_events FOR INSERT
  WITH CHECK (auth.role() = 'service_role');
CREATE POLICY "Service role only — update events"
  ON public.security_events FOR UPDATE
  USING (auth.role() = 'service_role');

CREATE POLICY "Service role only — select alerts"
  ON public.security_alerts FOR SELECT
  USING (auth.role() = 'service_role');
CREATE POLICY "Service role only — insert alerts"
  ON public.security_alerts FOR INSERT
  WITH CHECK (auth.role() = 'service_role');
CREATE POLICY "Service role only — update alerts"
