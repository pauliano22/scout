...
// ─── locationMatch — regions & states (the "Finance in Northeast = 0" bug) ──
console.log('\n═══ locationMatch (regions & states) ═══')
check('NYC is in the Northeast', locationMatch('New York, New York', ['Northeast']) === true)
check('Ithaca NY is in the Northeast (state component)', locationMatch('Ithaca, New York', ['Northeast']) === true)
check('AP-style "Pittsburgh, Pa." is in the Northeast', locationMatch('Pittsburgh, Pa.', ['Northeast']) === true)
check('two-letter "Boston, MA" is in the Northeast', locationMatch('Boston, MA', ['Northeast']) === true)
check('bare "Boston" is in the Northeast (metro fallback)', locationMatch('Boston', ['Northeast']) === true)
check('Seattle is NOT in the Northeast', locationMatch('Seattle, Washington', ['Northeast']) === false)
check('Chicago is NOT in the Northeast', locationMatch('Chicago, Illinois', ['Northeast']) === false)
check('"United States" suffix handled', locationMatch('New York, New York, United States', ['Northeast']) === true)
check('synonym "North East" works', locationMatch('Hartford, Connecticut', ['North East']) === true)
check('New England excludes NY', locationMatch('New York, New York', ['New England']) === false)
check('New England includes CT', locationMatch('Hartford, Connecticut', ['New England']) === true)
check('Tri-state includes NJ', locationMatch('Hoboken, New Jersey', ['Tri-State']) === true)
check('West Coast includes Seattle', locationMatch('Seattle, Washington', ['West Coast']) === true)
check('Midwest includes Ohio', locationMatch('Cincinnati, Ohio', ['Midwest']) === true)
check('bare state target: California matches "Sacramento, CA"', locationMatch('Sacramento, CA', ['California']) === true)
check('bare state target: Texas matches AP "Dallas, Tex."', locationMatch('Dallas, Tex.', ['Texas']) === true)
check('state target does NOT cross-match: California vs Portland OR', locationMatch('Portland, Oregon', ['California']) === false)
check('"New York" stays a CITY target (Ithaca still excluded)', locationMatch('Ithaca, New York', ['New York']) === false)
check('"Washington" stays a CITY target (Seattle still excluded)', locationMatch('Seattle, Washington', ['Washington']) === false)
check('Newark, DE is NOT Northeast (state decides; no NYC-alias bleed)', locationMatch('Newark, Delaware', ['Northeast']) === false)
check('Newark, NJ IS Northeast (state decides)', locationMatch('Newark, New Jersey', ['Northeast']) === true)
check('target synonym: Bay Area → SF metro', locationMatch('Palo Alto, California', ['Bay Area']) === true)
check('target synonym: NYC → New York metro', locationMatch('Brooklyn, NY', ['NYC']) === true)
check('target synonym: DC → Washington metro', locationMatch('Arlington, Virginia', ['DC']) === true)
